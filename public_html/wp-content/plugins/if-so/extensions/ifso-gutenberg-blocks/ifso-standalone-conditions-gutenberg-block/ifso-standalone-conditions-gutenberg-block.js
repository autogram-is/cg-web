( function( blocks, editor,  element ) {
    ...
    var forbidden_blocks = (typeof(forbidden_blocks_json)==='string') ? JSON.parse(forbidden_blocks_json) : forbidden_blocks_json;
    ...
    function is_block_forbidden(name){
        return forbidden_blocks.includes(name);
    }


    wp.hooks.addFilter( 'blocks.registerBlockType', 'ifso/ifso-standalone-conditions-block-filter', function(opts,name){
        if(is_block_forbidden(name)) return opts;
        opts.attributes = {
            ...opts.attributes,
            ifso_condition_type:{
                type:'string',
    ...
    var withIfSoSidebar = wp.compose.createHigherOrderComponent( function( BlockEdit ) {
        return function( props ) {
            ...
            if(!props.isSelected || is_block_forbidden(props.name)) return el(BlockEdit,props);
            return el(
                wp.element.Fragment,
                {},
                [
